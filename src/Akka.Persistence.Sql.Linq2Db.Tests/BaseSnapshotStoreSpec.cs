...
namespace Akka.Persistence.TCK.Snapshot
{
    ...
    /// </summary>
    public abstract class BaseSnapshotStoreSpec : PluginSpec
    {
        ...
        protected BaseSnapshotStoreSpec(Config config = null, string actorSystemName = null, ITestOutputHelper output = null) 
            : base(FromConfig(config).WithFallback(Config), actorSystemName ?? "SnapshotStoreSpec", output)
        {
            _senderProbe = CreateTestProbe();
        }

        protected BaseSnapshotStoreSpec(Type snapshotStoreType, string actorSystemName = null)
        ...
        public void SnapshotStore_should_load_the_most_recent_snapshot()
        {
            ...
            Assert.True(
                ...
                && LaxMetadataCompare(result.Snapshot.Metadata,Metadata[4])
                && result.Snapshot.Snapshot.ToString() == "s-5");
        }

        bool LaxMetadataCompare(SnapshotMetadata first, SnapshotMetadata other)
        {
            return string.Equals(first.PersistenceId, other.PersistenceId) &&
                   first.SequenceNr == other.SequenceNr &&
                   (Math.Abs((first.Timestamp - other.Timestamp)
                       .TotalMilliseconds) < 1);
        }
        ...
        public void SnapshotStore_should_load_the_most_recent_snapshot_matching_an_upper_sequence_number_bound()
        {
            ...
            _senderProbe.ExpectMsg<LoadSnapshotResult>(result =>
                result.ToSequenceNr == long.MaxValue
                && result.Snapshot != null
                && LaxMetadataCompare(result.Snapshot.Metadata,Metadata[2])
        ...
        public void SnapshotStore_should_delete_a_single_snapshot_identified_by_SequenceNr_in_snapshot_metadata()
        {
            ...
            _senderProbe.ExpectMsg<LoadSnapshotResult>(result =>
                result.ToSequenceNr == long.MaxValue
                && result.Snapshot != null
                && LaxMetadataCompare(result.Snapshot.Metadata,Metadata[1])
        ...
        public void SnapshotStore_should_delete_all_snapshots_matching_upper_sequence_number_and_timestamp_bounds()
        {
            ...
            var result = _senderProbe.ExpectMsg<LoadSnapshotResult>();
            Assert.True((result.ToSequenceNr == long.MaxValue
                         && result.Snapshot != null
                         && LaxMetadataCompare(result.Snapshot.Metadata, Metadata[3])
